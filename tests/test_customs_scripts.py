...
def test_generate_declaration_table():
    ...
    _returncode, stdout, _stderr = run_script(
        "generate_declaration_table.py", [test_data_path, test_output, "json"]
    )
    print(stdout)
...
def test_compare_documents():
    ...
    returncode, stdout, _stderr = run_script(
        "compare_documents.py", [invoice_path, packing_path]
    )
    print(stdout)
...
def test_update_source_manifest():
    ...
    manifest_path = os.path.join(
        ...
        "..",
        "..",
        "china-customs-declaration-project",
        "research",
    ...
    returncode, stdout, stderr = run_script(
        "update_source_manifest.py", ["--manifest", manifest_path, "--status"]
    )
    print(stdout)
    ...
    # 测试 --remind
    returncode, stdout, _stderr = run_script(
        "update_source_manifest.py", ["--manifest", manifest_path, "--remind"]
    )
    print(stdout)
    assert returncode == 0, "remind命令应成功"
...
if __name__ == "__main__":
    ...
    try:
        results.append(
            ("generate_declaration_table", test_generate_declaration_table())
        )
    except Exception as e:
